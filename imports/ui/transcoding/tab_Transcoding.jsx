...
import {ClientDB, GlobalSettingsDB, StatisticsDB} from '../../api/tasks.js';
...
class App extends Component {
  ...
  renderSlider(slider, sliderColour, sliderColour2) {
    ...
    return this.props.globalSettings.map((item, i) => (
      <span key={`gs-${i}`} className="sliderWidth">
        <center>
          {' '}
          <p>
            {title}({item[slider]})
  ...
  renderCheckBox = type =>
    this.props.globalSettings.map((item, i) => (
      <Checkbox
        key={`check-box-${i}`}
        name={type}
        checked={item[type]}
  ...
  renderTable(table, type, mode) {
    ...
    if (tables.length == 0) {
      data = [{_id: 1, file: 'Loading...', value: 1}];

      return data.map((row, i) => (
        <tr key={`tbl-row-${i}`} align="center">
          <td>
            <ClipLoader
              sizeUnit={'px'}
              size={25}
      ...
      if (mode == 'TranscodeDecisionMaker') {
        return data.map((row, i) => {
          ...
          if (row.file_size != undefined) {
            file_size = parseFloat((row.file_size / 1000).toPrecision(4));
          }
          ...
          return (
            <tr key={row._id}>
              ...
              <td>
                <p>{row.video_codec_name}</p>
              </td>
              <td>
                <p>{row.video_resolution}</p>
              </td>
              <td>
                <p>{file_size}</p>
              </td>
              <td>
                <p>
                  {!(row.bumped instanceof Date)
                    ? this.renderBumpButton(row.file)
                    : this.renderCancelBumpButton(row.file)}
                </p>
              </td>
            </tr>
          );
    ...
    if (type == 'success') {
      if (mode == 'TranscodeDecisionMaker') {
        return data.map((row, i) => {
          let lastTranscodeDate = '-';
          let oldSize = '-';
          let newSize = '-';
          if (row.lastTranscodeDate != undefined) {
            lastTranscodeDate = this.toTime(row.lastTranscodeDate);
          }

          if (row.oldSize != undefined) {
            oldSize = parseFloat(row.oldSize.toPrecision(4));
          }

          if (row.newSize != undefined) {
            newSize = parseFloat(row.newSize.toPrecision(4));
          }
          ...
          return (
            <tr key={row._id}>
              ...
              <td>
                <p>{row.TranscodeDecisionMaker}</p>
              </td>
              <td>
                <p>{oldSize}</p>
              </td>
              ...
              <td>
                <p>{this.renderRedoButton(row.file, mode)}</p>
              </td>
              <td>
                <p>{this.renderInfoButton(row.cliLog)}</p>
              </td>
              <td>
                <p>{this.renderHistoryButton(row)}</p>
              </td>
            </tr>
          );
        });
        ...
        return data.map((row, i) => {
          ...
          if (row.lastHealthCheckDate != undefined) {
            lastHealthCheckDate = this.toTime(row.lastHealthCheckDate);
          }
        ...
        return data.map((row, i) => {
          ...
          return (
            <tr key={row._id}>
              ...
              <td>
                <p>{this.renderIgnoreButton(row.file, mode)}</p>
              </td>
              <td>
                <p>{this.renderInfoButton(row.cliLog)}</p>
              </td>
  ...
  renderBumpButton(file) {
    ...
    return (
      <ItemButton file={file} obj={obj} symbol={'↑'} type="updateDBAction" />
    );
  }
  ...
  renderCancelBumpButton(file) {
    ...
    return (
      <ItemButton file={file} obj={obj} symbol={'X'} type="updateDBAction" />
    );
  }
  ...
  renderInfoButton(cliLog) {
    try {
      ...
      cliLog = cliLog.map(row => <p key={row}>{row}</p>);
          ...
          <div className="modalContainer">
            <div className="frame">
              <div className="scroll">
                <div className="modalText">{cliLog}</div>
              </div>
            </div>
          </div>
        </Modal>
  ...
  renderHistoryButton(row) {
    if (row.history == undefined) {
      ...
      result = result.map((row, i) => (
        <Markup key={`markup-${row}-${i}`} content={row} />
      ));
    }
  ...
  resetAllStatus(mode) {
    if (confirm('Are you sure you want to re-queue all files?')) {
      Meteor.call('resetAllStatus', 'all', mode);
    }
  }
  ...
  render() {
    ...
    <option  onClick={() => console.log("here2")}>Test2</option>
        ...
        <div className="container">
                  ...
                  <div className="frame">
                    <div className="scroll">
                      <div className="modalText">
                        <p>
                          Use the sliders to tell Tdarr to start up and maintain
                          the specified number of workers.{' '}
                        </p>
                        <p>
                          Workers which are toggled 'Off' will finish their
                          current item before closing down.
                        </p>
                        <p>
                          If you cancel an item, the worker will move onto the
                          next item in the queue.
                        </p>

                        <p></p>
                        <p>
                          Workers process newest items first and cycle between
                          your libraries.
                        </p>

                        <p>
                          General workers process both transcode and health
                          check items. They prioritise health check{' '}
                        </p>
                        <p></p>
                        <p></p>
                        <p></p>
                        ...
                        <p>
                          Important: Workers will not process items unless they
                          are within the scheduled times set in the library
                          settings.
                        </p>
                      </div>
                    </div>
            ...
            <center>
              <div style={ButtonStyle} className="workerButtoncontainer">
                <Button
                  ...
                  onClick={() => {
                    if (
                      confirm('Are you sure you want to cancel all workers?')
                    ) {
                      Meteor.call('getWorkers', function(error, result) {
                        var workers = result;

                        for (var i = 0; i < workers.length; i++) {
                          Meteor.call('killWorker', workers[i]._id);
                        }
                      });
                    }
                  }}
                >
                  <span className="buttonTextSize">Cancel all workers</span>
                </Button>
                ...
                <div style={ButtonStyle}>
                  <span className="buttonTextSize">
                    Low CPU priority:
                    <div style={ButtonStyle}>{this.renderLowCPUButton()}</div>
                  </span>
                </div>
              </div>
            </center>
          ...
          <div className="allWorkersContainer" id="allWorkersContainerID"></div>
        </div>
        <p></p>
        <p></p>
        <p></p>
        ...
        <div className="tabWrap">
            ...
            <div className="modalContainer">
              <div className="frame">
                <div className="scroll">
                  <div className="modalText">
                    <p>
                      All newly scanned files will be placed in the transcode
                      and health check queues.
                    </p>
                    ...
                    <p>
                      For transcodes, if files already meet the codec (etc)
                      requirements they will be marked as 'Not required' in
                      'Transcode: Success/Not required' tab.
                    </p>

                    <p>
                      If files don't meet the requirements they will be
                      transcoded. If transcoding is successful files will be
                      marked as 'Transcode success' in the 'Transcode:
                      Success/Not required' tab.
                    </p>
                    <p></p>

                    <p>
                      If transcoding fails or is cancelled then files will be
                      marked accordingly in the 'Transcode: Error/Cancelled'
                      tab.
                    </p>
                    ...
                    <p>
                      <b>
                        The end goal of Tdarr is to be able to run it on your
                        library and all items come out as 'Transcode:Not
                        required', meaning nothing needed to be
                        transcoded/remuxed etc.
                      </b>
                    </p>
                  </div>
                </div>
          ...
          <Tabs>
            <TabList>
              <Tab>
                <p>Transcode queue ({this.renderStat('table1Count')})</p>
              </Tab>
              <Tab>
                <p>
                  Transcode: Success/Not required (
                  {this.renderStat('table2Count')})
                </p>
              </Tab>
              <Tab>
                <p>
              ...
              <Tab>
                <p>Health check queue ({this.renderStat('table4Count')})</p>
              </Tab>
              <Tab>
                <p>Health check: Healthy ({this.renderStat('table5Count')})</p>
              </Tab>
              <Tab>
                <p>
                  Health check: Error/Cancelled (
                  {this.renderStat('table6Count')})
                </p>
              </Tab>
            </TabList>
                      ...
                      <th>
                        <p>
                          <Button
                            variant="outline-light"
                            onClick={() =>
                              this.resetAllStatus('TranscodeDecisionMaker')
                            }
                          >
                            <span className="buttonTextSize">Re-queue</span>
                          </Button>
                        </p>
                      </th>
                      <th>
