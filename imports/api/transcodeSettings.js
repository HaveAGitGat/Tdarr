...
Meteor.methods({
  ...
  removePluginInclude(DB_id, ele) {
    SettingsDB.update({ _id: DB_id }, { $pull: { pluginIDs: { _id: ele } } });
  },
  ...
  addVideoCodecExclude(DB_id, ele) {
    ...
    for (var i = 0; i < settings.length; i++) {
      if (
        settings[i].decisionMaker.video_codec_names_exclude.filter(
          (row) => row.codec === ele
        ).length == 0
      ) {
        SettingsDB.update(
  ...
  addAudioCodecExclude(DB_id, ele) {
    ...
    for (var i = 0; i < settings.length; i++) {
      if (
        settings[i].decisionMaker.audio_codec_names_exclude.filter(
          (row) => row.codec === ele
        ).length == 0
      ) {
        SettingsDB.update(
